...
@main.command()
@click.option("--file-path", type=click.Path(exists=True), help="Path to the SQL file you want to watch.")
def start(file_path):
    if file_path:
        fst_query.watch_directory(fst_query.CURRENT_WORKING_DIR, fst_query.handle_query, file_path)
    else:
